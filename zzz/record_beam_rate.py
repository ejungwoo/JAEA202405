...
def get_file_index(runNo):
    log_files = [f for f in os.listdir("beam") if re.match(rf"RUN{runNo:03}.beam_rate.\d+\.log",f)]
    if not log_files:
        return 0

    indices = []
    for f in log_files:
        matched=re.search(rf"RUN{runNo:03}.beam_rate.(\d+)\.log",f)
        if matched:
...
def setup_logger(runNo):
    ...
    logname = f"beam/RUN{runNo:03}.beam_rate.{index}.log"
    print()
