...
{
    fFiredDetector = new short[fNumDetectors];
    fFiredDCh = new short*[fNumDetectors];
    for (int det=0; det<fNumDetectors; ++det)
        fFiredDCh[det] = new short[NUM_MAX_DCH];
    ResetFired();
    ...
    for (auto midx=0; midx<fNumModules; ++midx)
        for (auto mch=0; mch<fNumMChannels; ++mch)
            fFxEnergyConversion[midx][mch] = nullptr;

    fDetector = new DetectorSetting();
}
...
void Analysis::AddAlphaCalibrationFile(TString name)
{
    ...
    for (int midx=0; midx<fNumModules; ++midx) {
        for (int iChannel=0; iChannel<fNumMChannels; ++iChannel) {
...
void Analysis::InitializeDrawing()
{
    ...
    {
        //fCvsOnline2 = new TCanvas("cvsOnline",Form("[RUN%03d] online update canvas 2",fRunNo),30,30,1600,900);
        //fCvsOnline2 -> Divide(2,2);

        fCvsOnline = new TCanvas("cvsOnline",Form("[RUN%03d] online update canvas",fRunNo),30,30,1600,900);
        ...
        fVPadEventCountInTime = fCvsOnline -> cd(3) -> cd(2);
                                fCvsOnline -> cd(4) -> Divide(1,2,0.02,0.02);
        fVPadStripCountInTime = fCvsOnline -> cd(4) -> cd(1);
        fVPadRatioCountInTime = fCvsOnline -> cd(4) -> cd(2);

        fVPadChCount          = fCvsOnline -> cd(1);

        fVPadADC              = fCvsOnline -> cd(5);
        fVPadEVSCh            = fCvsOnline -> cd(5);
        fVPadEVSAngle1        = fCvsOnline -> cd(6);

        fVPadEVSAngle2        = fCvsOnline -> cd(7);
        //fVPadLocalCountInTime = fCvsOnline -> cd(4) -> cd(2);
        //fVPadEVSS1Strip     = fCvsOnline -> cd(7);
        //fVPadEVSS3Strip     = fCvsOnline -> cd(8);
        fVPaddEVSE          = fCvsOnline -> cd(8);
        //fVPadEx             = fCvsOnline -> cd(8);

        fVPadEVSCh -> SetLogz();
        fVPadEVSAngle1 -> SetLogz();
        //fVPaddEVSE -> SetLogz();
    }
    ...
    fHistEVSAngle1  = new TH2D("hist_E_Angle"  , ";angle (deg);e",        20,20,40,fNumEE,0,fMaxEE);
    fHistEVSAngle2  = new TH2D("hist_E_Angle"  , ";angle (deg);e",        20,158,178,fNumEE,0,fMaxEE);
    ...
    fNumSelectedS1Strips = fS1ChosenS1Strips.size();
    for (int iStrip=0; iStrip<fNumSelectedS1Strips; ++iStrip) {
    ...
    if (fUpdateDrawingEveryNEvent>0) {
        ...
        SetAttribute(fHistEVSAngle1, fVPadEVSAngle1,1,true);
        SetAttribute(fHistEVSAngle2, fVPadEVSAngle2,1,true);
...
void Analysis::WriteRunParameters(TFile* file, int option)
{
    ...
    (new TNamed("beamEnergy", Form("%f",fBeamEnergy))) -> Write();
    if (option==0) {
        (new TNamed(Form("fUpdateDrawingEveryNEvent: %lld;", fUpdateDrawingEveryNEvent),"")) -> Write();
        ...
        (new TNamed(Form("fEventCountLimit         : %lld;", fEventCountLimit         ),"")) -> Write();
...
void Analysis::ReadDataFile()
{
    ...
    short FENumber, mch, adc, tsGroup;
    ...
    while (true)
    {
        ...
        while (fDataFile >> buffer)
        {
            ...
            short midx = fDetector -> FENumber_MIdx(FENumber);
            int gid = GetGlobalID(midx, mch);
            double energy = 0;
            if (fEnergyConversionIsSet)
                energy = GetCalibratedEnergy(midx,mch,adc);
            auto det = fDetector -> MIdxMCh_Det(midx,mch);
            auto dch = fDetector -> MIdxMCh_DCh(midx,mch);
...
bool Analysis::FillDataTree()
{
    ...
    int groupdE = 0;
    int groupS1 = 0;
    if ((fdES1CoincidenceMode || fdES1S3CoincidenceMode) && fGoodCoincidenceEvent)
    {
        auto chdE = (ChannelData*) fChannelArray -> At(fFiredDetector[kdE]);
        auto chS1 = (ChannelData*) fChannelArray -> At(fFiredDetector[kS1J]);
        auto S1dCh = chS1->dch;
        groupdE = fDetector -> MIdxMCh_Group(chdE->midx,chdE->mch);
        groupS1 = fDetector -> MIdxMCh_Group(chS1->midx,chS1->mch);
        bdE = chdE->energy;
        ...
        if (groupdE==groupS1)
        {
            ...
            if (fTritonCutGIsSet)
            {
                auto detectorTheta = fDetector -> S1Ch_Angle(S1dCh);
                if (fBeamEnergy==0)
    ...
    if (fillEVSAngle)
    {
        for (auto iChannel=0; iChannel<fChannelArray->GetEntries(); ++iChannel)
        {
            ...
            if (ch->det==kS1J) {
                double angle1 = fDetector->S1Ch_Angle1(ch->dch);
                double angle2 = fDetector->S1Ch_Angle2(ch->dch);
                fHistEVSS1Strip -> Fill(fDetector->S1Ch_Strip(ch->dch),ch->energy);
                fHistEVSAngle1 -> Fill(gRandom->Uniform(angle1,angle2),ch->energy);
            }
            if (ch->det==kS3J) {
                double angle1 = fDetector->S3Ch_Angle1(ch->dch);
                double angle2 = fDetector->S3Ch_Angle2(ch->dch);
                fHistEVSAngle2 -> Fill(gRandom->Uniform(angle1,angle2),ch->energy);
            }
        }
    }
    /////////////////////////////////////////////////////////////////////////
...
void Analysis::FillLocalHistograms()
{
    ...
    for (auto iChannel=0; iChannel<fChannelArray->GetEntries(); ++iChannel)
    {
        ...
        if (ch->det==kS1J)
        {
            auto strip = fDetector->S1Ch_Strip(ch->dch);
            bool found = false; for (int i=0; i<fNumSelectedS1Strips; ++i) { if (fS1ChosenS1Strips[i]==strip) { found = true; break; } }
            if (found) {
                if (fEnergyCutG!=nullptr) {
                    if (fEnergyCutG->IsInside(fDetector->S1Ch_Angle(ch->dch),ch->energy)==true) {
...
bool Analysis::AskContinueRun(TString message)
{
    ...
    if (fAutoUpdateRun) {
        ...
        cout << "\033[0;32m" << Form("== (%lld) Enter / stop / auto / wait: ",fCountEvents) << "\033[0m";
...
bool Analysis::AskUpdateDrawing(TString message)
{
    ...
    if (fAutoUpdateDrawing) {
        ...
        if (!message.IsNull()) couti << message << endl;
        cout << "\033[0;32m" << Form("== (%lld) Enter / stop / auto / all: ",fCountEvents) << "\033[0m";
        std::getline(std::cin, userInput0);
    }
    TString userInput = userInput0;
    userInput.ToLower();
    ...
    else
    {
        if (userInput=="auto")
        {
            ...
            couti << Form("automatically update every %lld events",fUpdateDrawingEveryNEvent) << endl;
        }
        if (userInput=="all")
        {
        ...
        if (userInput.IsDec() && userInput.Atoi()>0)
        {
            ...
            couti << Form("Reading next %lld events",fUpdateDrawingEveryNEvent) << endl;
        }
    }
...
void Analysis::UpdateCvsOnline(bool firstDraw, TFile* file)
{
    ...
    auto DrawModuleBoundary = [this](double yMax)
    {
        for (int midx=0; midx<fNumModules; ++midx) {
            if (midx!=0) {
                auto line = new TLine(midx*fNumMChannels,0,midx*fNumMChannels,yMax);
                line -> SetLineColor(kBlack);
                line -> Draw("samel");
            }
            for (int iDiv : {1,2,3}) {
                auto line = new TLine(midx*fNumMChannels+iDiv*4,0,midx*fNumMChannels+iDiv*4,yMax*0.5);
                line -> SetLineColor(kGray+1);
                line -> SetLineStyle(2);
                line -> Draw("samel");
            }
            auto det = fDetector -> MIdxMCh_Det(midx,0);
            TString detectorName = TString(fDetector->GetDetectorName(det)(0,3));
            auto tt = new TText((midx+0.5)*fNumMChannels,-yMax*0.1,detectorName);
            tt -> SetTextAlign(22);
            tt -> Draw("same");
        }
    };
    ...
    if (fVPadEVSAngle1 !=nullptr) { fVPadEVSAngle1  -> cd(); fHistEVSAngle1  -> Draw("colz"); }
    if (fVPadEVSAngle2 !=nullptr) { fVPadEVSAngle2  -> cd(); fHistEVSAngle2  -> Draw("colz"); }
    if (file!=nullptr) fHistEVSS1Strip -> Write();
    if (file!=nullptr) fHistEVSS3Strip -> Write();
    if (file!=nullptr) fHistEVSAngle1   -> Write();
...
void Analysis::PrintConversionSummary(TFile* file)
{
    ...
    couti << Form("#events with 0 ch : %d",  fCoincidenceCount[0]) << endl;
    couti << Form("#events with 1 ch : %d",  fCoincidenceCount[1]) << endl;
    couti << Form("#events with 2 ch : %d",  fCoincidenceCount[2]) << endl;
    couti << Form("#events with 3 ch : %d",  fCoincidenceCount[3]) << endl;
    couti << Form("#events with > ch : %d",  fCoincidenceCount[4]) << endl;
    ...
    if (file!=nullptr) {
        ...
        (new TNamed(Form("#events with 0 ch : %d  ;",  fCoincidenceCount[0]),"")) -> Write();
        (new TNamed(Form("#events with 1 ch : %d  ;",  fCoincidenceCount[1]),"")) -> Write();
        (new TNamed(Form("#events with 2 ch : %d  ;",  fCoincidenceCount[2]),"")) -> Write();
        (new TNamed(Form("#events with 3 ch : %d  ;",  fCoincidenceCount[3]),"")) -> Write();
        (new TNamed(Form("#events with > ch : %d  ;",  fCoincidenceCount[4]),"")) -> Write();
    }
}

int Analysis::GetGlobalID(short midx, short mch)
{
    int globalID = midx*fNumMChannels + mch;
    return globalID;
}
...
{
    int det = fDetector -> MIdxMCh_Det(midx,mch);
    int dch = fDetector -> MIdxMCh_DCh(midx,mch);
    TString detectorTitle = fDetector -> GetDetectorName(det);
    if (addChannel)
